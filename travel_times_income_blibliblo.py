...
def plot_bin_normalized_travel_time_by_income_bin_lines(
    algo_runs,
    episode_range=None,
    episodes=None,
    last_n=30,
    ...
    Plots, for each scenario and income bin:
        ...
        E[travel_time | income_bin] / E[travel_time]

    where the denominator is the overall mean travel time across the whole
    population in that run.
    """
    ...
    for sc in scenarios:
        ...
        for run_dir in algo_runs[sc]:
            ...
            tmp = tmp.dropna(subset=[id_col, income_col, "travel_time_used"])
            tmp = tmp[tmp[income_col] > 0]
            if tmp.empty:
                continue
            ...
            if avg_mode == "agent":
                per_agent = (
                    tmp.groupby([id_col, "income_bin"], observed=True)["travel_time_used"]
                       .mean()
                       .reset_index()
                )
                bin_series = per_agent.groupby("income_bin", observed=True)["travel_time_used"].mean()
                overall_mean = per_agent["travel_time_used"].mean()
            else:
                bin_series = tmp.groupby("income_bin", observed=True)["travel_time_used"].mean()
                overall_mean = tmp["travel_time_used"].mean()

            if pd.isna(overall_mean) or overall_mean == 0:
                print(f"[WARN] {sc} / {run_dir}: overall mean travel time is zero/NaN. Skipping run.")
                continue
    ...
    ax.set_xlabel("Income bin", fontsize=13)
    ax.set_ylabel("Bin-normalized travel time", fontsize=13)
    ax.grid(True, axis="y", alpha=0.25)
    ax.legend(frameon=False, fontsize=11)
...
def plot_total_travel_time_by_income_bin_lines(
    algo_runs,
    episode_range=None,
    episodes=None,
    last_n=30,
    ...
    # "row" = sum over all rows in each bin
    # "agent" = sum per agent first, then average those sums across agents in bin
    avg_mode="row",
    ...
    Plots, for each scenario and income bin:
        ...
        sum(travel_time | income_bin)

    If avg_mode == "row":
        total travel time in the bin across all rows.

    If avg_mode == "agent":
        first sum travel time per agent within each bin, then average those
        agent totals within the bin.
    """
    ...
    for sc in scenarios:
        ...
        for run_dir in algo_runs[sc]:
            ...
            if avg_mode == "agent":
                ...
                bin_series = tmp.groupby("income_bin", observed=True)["travel_time_used"].sum()

            vec = bin_series.reindex(bin_labels).to_numpy(dtype=float)
            per_run_vecs.append(vec)
    ...
    ax.set_xlabel("Income bin", fontsize=13)
    ax.set_ylabel("Total travel time", fontsize=13)
    ax.grid(True, axis="y", alpha=0.25)
    ax.legend(frameon=False, fontsize=11)
...
plot_bin_normalized_travel_time_by_income_bin_lines(
    ALGO_RUNS,
    episode_range=(1000, 1340),
    income_col="income",
    travel_time_col="travel_time",
    travel_time_unit="minutes",
    plot_kind="AV",
    ...
    smooth_window=3,
    show_std_band=True,
    save_path="imgs/bin_normalized_travel_time_by_income_bin.png",
    show=True,
)