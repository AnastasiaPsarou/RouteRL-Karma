...
from routerl_karma.environment import generate_agents
...
from routerl_karma.utilities import get_params
...
class TrafficEnvironment(ParallelEnv):
    ...
    def _initialize_machine_agents(self) -> None:
        ...
        nvec = np.array([
            self.simulation_params[kc.NUMBER_OF_PATHS],           # e.g., N paths
            self.simulation_params[kc.SIMULATION_TIMESTEPS]       # M discrete time choices
        ], dtype=np.int64)

        if self.karma_pricing: 
            self.action_spaces = {
                agent: Box(low=0, high=self.simulation_params[kc.SIMULATION_TIMESTEPS], shape=(self.simulation_params[kc.SIMULATION_TIMESTEPS], self.simulation_params[kc.NUMBER_OF_PATHS]), dtype=np.int32)
    ...
    def step(self, machine_actions: int = None) -> None:
        """Step method.
        ...
        if self.karma_pricing:
            # Transform the biddings into action format
            print("machine actions before are: ", machine_actions, "\n\n")
            machine_actions = self.karma_auction(1, machine_actions)
            print("machine_actions are: ", machine_actions, "\n\n")
        ...
        observations = self.observation_obj(self.agents)
        self.day = self.day + 1
        self._reset_episode()
    ...
    def karma_auction(self, x, agent_bids) -> tuple:
        """
    ...
    def _help_step(self, actions: list[tuple]) -> dict:
        ...
        for agent, action in actions:
            action_dict = {kc.AGENT_ID: agent.id,
                           ...
                           kc.AGENT_START_TIME: self.simulator.timestep, #vehicles depart based on the simulation 
                           kc.INCOME: agent.income} #step and not based on their departure time
    ...
    def simulation_loop(self, machine_actions: dict) -> None:
        ...
        id_to_human  = {human.id: human for human in self.human_agents}
        id_to_machine = {machine.id: machine for machine in self.machine_agents}
        ...
        while (
            ...
            # --- Humans ---
            for agent_id_str, action in human_actions.items():
                # action[1] is departure time
                if action[1] == self.simulator.timestep:
                    agent_obj = id_to_human[int(agent_id_str)]
                    actions_for_timestep.append((agent_obj, action[0]))

            # --- Machines ---
            for agent_id_str, action in sorted_machines.items(): ###### The actions of the machines are not properly parsed in the env
                if action[1] == self.simulator.timestep:
                    agent_obj = id_to_machine[int(agent_id_str)]
                    actions_for_timestep.append((agent_obj, action[0]))

            travel_times = self._help_step(actions_for_timestep)
