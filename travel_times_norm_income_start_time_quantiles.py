...
def compute_tt_minus_best_observed_route(
    ...
    travel_time_unit="minutes",   # "minutes" or "seconds"
    output_col="tt_diff",
    use_absolute=True,            # <- NEW
):
    """
    Creates a new column:
        experienced_travel_time - min(first 3 values of observations) * obs_tt_scale

    If use_absolute=True, stores the absolute value:
        |experienced_travel_time - best_observed_travel_time|
...
def build_hourly_income_bins(
    run_dirs,
    n_equal_bins=5,   # <- quintiles
    episode_range=None,
    episodes=None,
    last_n=30,
    income_col="income",
    ...
    """
    Build global hourly-income quintile bins across all runs.

    hourly_income = income / work_hours_per_day
    Returns:
        edges: bin edges
        labels: quantile labels (Q1, ..., Q5)
    """
    pooled_hourly_income = []

    for rd in run_dirs:
        df = load_episodes(rd, n_last=last_n, episode_range=episode_range, episodes=episodes)
        if df is None or df.empty or income_col not in df.columns:
            print(f"[WARN] Could not derive hourly income from {rd}. Skipping run.")
            continue

        tmp = pd.to_numeric(df[income_col], errors="coerce").dropna()
        if tmp.empty:
            continue

        hourly_income = tmp / float(work_hours_per_day)
        vals = pd.to_numeric(hourly_income, errors="coerce").dropna().to_numpy()
        if vals.size:
            pooled_hourly_income.append(vals)

    if not pooled_hourly_income:
        raise ValueError("No valid hourly income values found across runs.")

    pooled_hourly_income = np.concatenate(pooled_hourly_income)

    qs = np.linspace(0, 1, n_equal_bins + 1)
    edges = np.unique(np.quantile(pooled_hourly_income, qs))

    if len(edges) < 2:
        raise ValueError("Quantile edges collapsed for hourly income bins.")

    labels = [f"Q{i}" for i in range(1, len(edges))]
    return edges, labels


def plot_tt_minus_best_observed_by_start_quantile_and_hourly_income_bars(
    algo_runs,
    episode_range=None,
    episodes=None,
    last_n=30,
    ...
    # hourly-income binning
    n_hourly_income_bins=4,
    work_hours_per_day=8.0,
    ...
    """
    Grouped barplot:
      x = start-time quantiles
      colors = hourly-income quartiles
      y = avg(experienced travel time - best observed travel time)

    Q1 on x-axis = earliest start times
    Q5 on x-axis = latest start times
    """

    scenarios = list(algo_runs.keys())
    n_panels = len(scenarios)

    # Build consistent hourly-income bins across all runs
    all_run_dirs = [rd for runs in algo_runs.values() for rd in runs]
    income_edges, income_labels = build_hourly_income_bins(
        all_run_dirs,
        n_equal_bins=n_hourly_income_bins,
        episode_range=episode_range,
        episodes=episodes,
        last_n=last_n,
        income_col=income_col,
    ...
    start_labels = [f"Q{i}" for i in range(1, n_start_quantiles + 1)]
    n_start_bins = len(start_labels)

    def assign_hourly_income_group(income_series: pd.Series) -> pd.Series:
        hourly_income = pd.to_numeric(income_series, errors="coerce") / float(work_hours_per_day)
        return pd.cut(
            hourly_income,
            bins=income_edges,
    ...
    x = np.arange(n_start_bins)
    bar_width = 0.18 if n_income_bins == 4 else 0.8 / max(n_income_bins, 1)
    offsets = (np.arange(n_income_bins) - (n_income_bins - 1) / 2.0) * bar_width
    ...
    for ax, sc in zip(axes, scenarios):
        ...
        for run_dir in algo_runs[sc]:
            ...
            # bar colors
            tmp["hourly_income_bin"] = assign_hourly_income_group(tmp[income_col])
            tmp = tmp.dropna(subset=["hourly_income_bin"])
            if tmp.empty:
                continue

            if avg_mode == "agent":
                per_agent = (
                    tmp.groupby([id_col, "start_q", "hourly_income_bin"], observed=True)["tt_diff"]
                       .mean()
                       .reset_index()
                )
                series = (
                    per_agent.groupby(["start_q", "hourly_income_bin"], observed=True)["tt_diff"]
                    .mean()
                )
            else:
                series = (
                    tmp.groupby(["start_q", "hourly_income_bin"], observed=True)["tt_diff"]
                    .mean()
                )

            mat = np.full((n_start_bins, n_income_bins), np.nan, dtype=float)
            for i_s, s_lab in enumerate(start_labels):
                for j_inc, inc_lab in enumerate(income_labels):
                    key = (s_lab, inc_lab)
                    if key in series.index:
                        mat[i_s, j_inc] = float(series.loc[key])
        ...
        if not per_run_mats:
            ax.set_title(sc + " (no data)")
            ax.set_xticks(x)
            ax.set_xticklabels(start_labels)
            ax.grid(True, axis="y", alpha=0.25)
            continue
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(start_labels)
        ax.set_xlabel("Start-time quantile")
        ax.grid(True, axis="y", alpha=0.25)
        ax.legend(title="Hourly income quartile", frameon=False)
...
plot_tt_minus_best_observed_by_start_quantile_and_hourly_income_bars(
    ALGO_RUNS,
    ...
    n_hourly_income_bins=5,   # quartiles
    work_hours_per_day=8.0,
    ...
    save_path="imgs/avg_tt_minus_best_observed_by_start_quantile_hourly_income_bars.png",
    show=True,
)