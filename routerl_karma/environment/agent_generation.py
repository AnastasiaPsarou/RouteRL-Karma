...
from routerl_karma.environment import HumanAgent
...
def generate_agents(params, free_flow_times, generate_data, seed=23423) -> list:
    ...
    mean_target = 2500
    std_target = 1500
    lower_bound = 0     # minimum income = 0
    upper_bound = np.inf  # no upper cap

    # Convert the bounds to the standardized normal space
    a, b = (lower_bound - mean_target) / std_target, (upper_bound - mean_target) / std_target

    # Create truncated normal distribution
    trunc_dist = truncnorm(a=a, b=b, loc=mean_target, scale=std_target)

    # Sample incomes
    income_normal = trunc_dist.rvs(size=len(agents_data_df))

    """ Plot income distribution

    plt.hist(income_normal, bins=50, alpha=0.7, edgecolor='black')
    plt.title("Truncated Normal Income Distribution")
    plt.xlabel("Income")
    ...
    for index, row in agents_data_df.iterrows():
