...
class LinearHyperNetwork(nn.Module):
    """
    Linear hypernetwork: generates weights/biases for each target layer with a single linear map.
    For each target layer (in_dim -> out_dim), it generates:
      W: (B, in_dim, out_dim)
      b: (B, out_dim)

    This mirrors the JAX idea: nn.Dense(emb -> weight_dim) + nn.Dense(emb -> bias_dim),
    no hidden layers, optional bias, layer-wise heads.
    """
    def __init__(
        self,
        embedding_dim: int,
        layer_dims: List[Tuple[int, int]],
        use_bias_in_hypernet: bool = False,   # JAX default was False for "strictly linear"
        final_gain_style: str = "actor",      # "actor" or "critic"
        init_scale: float = np.sqrt(2),       # for non-final layers
    ):
        super().__init__()
        self.layer_dims = layer_dims
        self.use_bias_in_hypernet = use_bias_in_hypernet
        ...
        for (in_dim, out_dim) in layer_dims:
            ...
            b_out = out_dim
            self.weight_heads.append(nn.Linear(embedding_dim, w_out, bias=use_bias_in_hypernet))
            self.bias_heads.append(nn.Linear(embedding_dim, b_out, bias=use_bias_in_hypernet))

        self._init_params()

    def _layer_gain(self, layer_idx: int) -> float:
        is_last = layer_idx == (len(self.layer_dims) - 1)
        if is_last and self.final_gain_style == "actor":
            return 0.01
        if is_last and self.final_gain_style == "critic":
            return 1.0
        return float(self.init_scale)

    def _init_params(self):
        # Initialize each head similarly to PPO common practice:
        # orthogonal weights, zero bias.
        for i, (w_head, b_head) in enumerate(zip(self.weight_heads, self.bias_heads)):
            gain = self._layer_gain(i)

            # weight head produces flattened W for the target layer
            nn.init.orthogonal_(w_head.weight, gain=gain)
            if w_head.bias is not None:
                nn.init.zeros_(w_head.bias)

            # bias head: in JAX code, kernel was zeros; keeping orthogonal is also ok,
            # but to match the "bias generator is simple" spirit, we zero-init kernel & bias.
            nn.init.zeros_(b_head.weight)
            if b_head.bias is not None:
    ...
    def forward(self, emb: torch.Tensor) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
        ...
        for (in_dim, out_dim), w_head, b_head in zip(self.layer_dims, self.weight_heads, self.bias_heads):
            w_flat = w_head(emb)                # (B, in_dim*out_dim)
            b = b_head(emb)                     # (B, out_dim)
            w = w_flat.view(-1, in_dim, out_dim)
            weights.append(w)
            biases.append(b)
...
class ActorCriticHyperIPPO(nn.Module):
    ...
    def __init__(
        ...
        self.actor_hnet = LinearHyperNetwork(
            embedding_dim=embedding_dim,
            layer_dims=self.actor_layer_dims,
            use_bias_in_hypernet=False,          # match JAX default
            final_gain_style="actor",
            init_scale=np.sqrt(2),
        )
        self.critic_hnet = LinearHyperNetwork(
            embedding_dim=embedding_dim,
...
def main():
    ...
    seed = args.seed
    records_folder = f"training_records_hyperippo_linear_300_agents_seed_{seed}_monetary_pricing"
    plots_folder = f"plots_hyperippo_linear_300_agents_seed_{seed}_monetary_pricing"
    ...
    """scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer,
        lr_lambda=lambdaa step: max(0.0, 1.0 - step / total_updates)
    )"""
