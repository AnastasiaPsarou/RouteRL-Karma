...
def compute_tt_minus_best_observed_route(
    ...
    observations_col="observations",
    obs_tt_scale=100.0,
    input_travel_time_unit="minutes",   # unit of CSV travel_time: "minutes" or "seconds"
    output_unit="seconds",              # "minutes" or "seconds"
    output_col="tt_diff",
):
    """
    Creates a new column:
    ...
    The result is returned in `output_unit`.
    Assumes observations[:3] * obs_tt_scale are in minutes originally.
    """
    if travel_time_col not in df.columns or observations_col not in df.columns:
        df[output_col] = np.nan
        return df

    travel_time = pd.to_numeric(df[travel_time_col], errors="coerce").astype(float)

    # Convert CSV travel_time to minutes first
    if input_travel_time_unit.lower().startswith("sec"):
    ...
    for obs in df[observations_col]:
        ...
        # observations are normalized; de-normalized baseline in minutes
        best_tt_min = np.min(first3) * obs_tt_scale
        if not np.isfinite(best_tt_min):
            mins.append(np.nan)
            continue

        mins.append(best_tt_min)

    best_route_tt_min = pd.Series(mins, index=df.index, dtype=float)

    # Difference in minutes
    diff_min = travel_time_min - best_route_tt_min

    # Convert output unit
    if output_unit.lower().startswith("sec"):
        df[output_col] = diff_min * 60.0
    else:
        df[output_col] = diff_min
...
def plot_tt_minus_best_observed_route_by_income_bin_lines(
    ...
    obs_tt_scale=100.0,
    input_travel_time_unit="minutes",   # unit in CSV
    output_unit="seconds",              # unit on plot
    avg_mode="row",
    ...
    for sc in scenarios:
        ...
        for run_dir in run_dirs:
            ...
            tmp = compute_tt_minus_best_observed_route(
                ...
                observations_col=observations_col,
                obs_tt_scale=obs_tt_scale,
                input_travel_time_unit=input_travel_time_unit,
    ...
    ax.tick_params(axis="y", labelsize=14)
    ax.set_xlabel("Income bin", fontsize=16)
    ax.set_ylabel(f"Avg experienced TT - best observed TT ({output_unit})", fontsize=16)   
...
if __name__ == "__main__":
    ...
    plot_tt_minus_best_observed_route_by_income_bin_lines(
        ...
        n_equal_bins=5,
        obs_tt_scale=100.0,
        input_travel_time_unit="minutes",   # your CSV travel_time unit
        output_unit="seconds",              # y-axis unit you want
        avg_mode="row",
        show_std_band=True,
        smooth_window=3,
        save_path="imgs/avg_tt_minus_best_observed_route_by_income_bin_lines_seconds.png",
        show=True,
    )
